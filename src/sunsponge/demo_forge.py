...
from sunsponge.demo_ai import (
    DemoAI,
    DemoAIError,
    DemoEnrichManager,
    _synthesize_one,
)
from sunsponge.demo_engine import DemoManager, run_async
...
def project_root() -> Path:
    """Resolve the SunSponge project root, honoring ``SUNSPONGE_ROOT``."""
    override = os.environ.get("SUNSPONGE_ROOT", "").strip()
    if override:
...
def viewer_template_path() -> Path:
    return project_root() / "viewer" / "demo-viewer.html"
